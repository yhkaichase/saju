...
src/
  ...
  lib/
    ...
    calendar/         # 음양력 변환·절기 계산
      ...
      true-solar-time.ts# 진태양시 보정 (경도보정 + 균시차) — 시주·일주 경계용
  types/              # 공용 타입 (saju.ts)
```
...
- ✅ **진태양시(眞太陽時) 보정 항상 적용**(서울 경도 고정): 시주 시지·일주 자시
  경계를 `표준시 + (경도−135°)×4분 + 균시차`로 계산. 연·월주·대운은 절대시각
  비교라 미보정. `buildSajuChart` 경계에서만 적용해 코어 골든/KASI 비교는 보존.
  ⚠️ 1908–11·1954–61 표준시(UTC+8:30, 127.5°) 구간은 미반영(현 사용 대상 밖).
